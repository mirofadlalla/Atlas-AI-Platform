...
def connect(
    ...
    existing = (
        db.query(TenantDatabase).filter_by(tenant_id=str(admin.tenant_id)).one_or_none()
    )
    ...
    try:
        ...
        db.rollback()
        raise HTTPException(
            status_code=400, detail={"code": "DATABASE_CONNECTION_FAILED"}
        )
    return {
        "status": "connected",
...
def test(admin=Depends(require_admin), db: Session = Depends(get_db)):
    try:
        return tenant_database_manager.test_connection(db, str(admin.tenant_id))
    except TenantDatabaseError as exc:
        _error(exc)
    except Exception:
        raise HTTPException(
...
def status(admin=Depends(require_admin), db: Session = Depends(get_db)):
    ...
    return {
        ...
        "database_type": config.database_type if config else None,
        "host_masked": "***" if config else None,
        "database_name": config.database_name if config else None,
        "schema": config.default_schema if config else None,
...
def refresh_schema(admin=Depends(require_admin), db: Session = Depends(get_db)):
    try:
        return tenant_database_manager.schema(db, str(admin.tenant_id), refresh=True)
    except TenantDatabaseError as exc:
        _error(exc)
    except Exception:
        raise HTTPException(
            status_code=400, detail={"code": "SCHEMA_INTROSPECTION_FAILED"}
        )
...
def get_schema(admin=Depends(require_admin), db: Session = Depends(get_db)):
    try:
        return tenant_database_manager.schema(db, str(admin.tenant_id))
    except TenantDatabaseError as exc:
        _error(exc)
    except Exception:
        raise HTTPException(
