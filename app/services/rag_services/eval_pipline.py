    ...
    try:
        if run_id:
            ...
            with mlflow.start_run(run_id=run_id):
                ...
                if retry_count > 0:
                    mlflow.set_tag(
                        f"retry_{retry_count}_attempt", datetime.utcnow().isoformat()
                    )
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        logger.debug(
                            f"Successfully sent evaluation metrics for tenant {tenant_id}"
                        )
                    else:
                        logger.warning(
                            f"API webhook returned status {resp.status_code} for eval metrics"
                        )
                except Exception as metric_error:
                    logger.error(
                        f"Error recording evaluation metrics via webhook: {metric_error}"
                    )
        ...
        try:
            ...
            with mlflow.start_run(run_id=run_id):
                ...
                mlflow.set_tag(
                    f"error_time_{retry_count}", datetime.utcnow().isoformat()
                )
...
def generate_eval_dataset_task(self, tenant_id: str, max_chunks: int = 30):
    from app.rag.evaluation.generate_eval_dataset import (
        fetch_points,
        build_llm,
        build_dataset,
    )
