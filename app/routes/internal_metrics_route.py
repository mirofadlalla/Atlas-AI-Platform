...
async def record_metrics(
    ...
    try:
        ...
        if metric_type == "query_run":
            ...
            agent_reasoning_duration_seconds.labels(agent_type="reasoning").observe(float(latency))
            agent_decision_duration_seconds.labels(agent_type="reasoning").observe(float(latency))
            ...
            document_ingestion_duration_seconds.labels(document_type=document_type).observe(float(latency))
            if chunks_created > 0:
                document_chunks_created.labels(document_type=document_type).observe(int(chunks_created))
