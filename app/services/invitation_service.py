...
class InvitationService:
    ...
    def send_invitation(
        self, invited_email: str, invited_by_id: str, tenant_id: str, **kwargs
    ):
        """
        ...
        active_invitation = (
            next(  # ده generator expression بيشوف لو في دعوة صالحة موجودة بنفس الإيميل
                (inv for inv in existing_invitations if inv.is_valid()), None
            )
        )
        ...
        try:
            ...
            logger.error(
                f"Failed to dispatch invitation email to {invited_email}: {mail_err}"
            )
    ...
    def accept_invitation_and_register(
        self, token: str, name: str, password_hash: str, tenant_id: str
    ) -> Optional[Users]:
        """
